...
def _register_mailbox_strategies():
    ...
    def _url_html_fetch(mb, *, limit, proxy, **kw):
        try:
            ...
            return mailbox_url_html.fetch_url_html_messages(
                mb, limit=limit, proxy="",
            )

    mailbox_strategies.register_message_fetcher(
        "url_html",
        lambda mb, cfg: str(getattr(mb, "provider", "") or "") == "url_html",
        _url_html_fetch,
    )
...
def _configured_mailbox_proxy(runtime_config: ConfigInput = None):
    ...
    email_cfg = _email_cfg() if runtime_config is None else _email_cfg(runtime_config)
...
def _snapshot_mailbox_message(mailbox, proxy=None):
    ...
    if provider in {"url_html", "cfworker", "remail", mailbox_icloud_url.PROVIDER}:
        try:
            if provider == mailbox_icloud_url.PROVIDER:
                ...
                messages = _fetch_mailbox_messages(
                    mailbox,
                    limit=25 if provider == "url_html" else 1,
                    proxy=proxy,
                )
...
def _poll_email_otp(
    ...
    *,
    ...
    cfg = _email_cfg() if runtime_config is None else _email_cfg(runtime_config)
