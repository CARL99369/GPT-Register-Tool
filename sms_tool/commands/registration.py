...
def registration_phone_pool(args: Any):
    ...
    from ..phone_reuse import create_phone_pool, has_phone_reuse_config, load_phone_pool_entries, print_phone_pool_status
    ...
    phone_pool_file = str(getattr(args, "phone_pool_file", "") or "").strip()
    if phone_pool_file:
        if str(getattr(args, "phone_source", "") or "") != "phone_pool":
            print("[Error] --phone-pool-file requires --phone-source phone_pool")
            raise SystemExit(2)
        try:
            explicit_entries = load_phone_pool_entries(phone_pool_file)
