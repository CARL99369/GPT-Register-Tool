...
def test_registered_remail_rows_can_build_one_click_sms_mailbox_files():
    ...
    # remail:// lines are routed through the shared format-aware parser and
    # still passed to the backend as mailbox files.
    assert "MailboxLineParser.TryParse" in source
    assert 'value.StartsWith("remail://"' in parser
    assert '"remail", "--mailbox-file", value' in parser
